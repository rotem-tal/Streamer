...
def main():
    ...
    if len(sys.argv) <= 1:
        ...
        parser.add_argument('--configuration', help='Path to StreamPlot Configuration file (.SCG)', required=True, dest='cfg')
        parser.add_argument('--subject_matrix', help='Path to subject microbiome reads', dest='sub')
        parser.add_argument('--delim', help='Optional: Subject matrix delimiter (default is \",\")', dest='delim1')
        parser.add_argument('--meta', help='Path to Meta-data file (as csv, xls, xlsx)', dest='meta')
        parser.add_argument('--meta_delimiter', dest='delim2', help='Metadata delimiter (default is \",\")')
        parser.add_argument('--meta_sheet', dest='sheet', help='Metadata sheet name (for Excel files)')
        parser.add_argument('--cluster', nargs=4, help=f'Cluster subjects and plot clusters, parameters:{os.linesep}Number of dominant bacteria to consider{os.linesep}Matching threshold{os.linesep}Time threshold{os.linesep}Delta', dest='c')
        parser.add_argument('--plot', help='Plot all inserted subjects', dest='p')
        parser.add_argument('--folder', dest='f', help='Folder to save to, default is \"Plots\"')
        parser.add_argument('--events', dest='events', nargs='*', help=f'Add events from the meta data. Optional arguments (as ordered):{os.linesep}Event name (default \"Onset\"){os.linesep}Meta data subject ID column (default \"record_id\"){os.linesep}Event columns, Time column and events identifier (e.g. \"Symptomatic\", \"Case\" etc.)')
